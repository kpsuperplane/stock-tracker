...
beforeEach(async () => {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("DELETE FROM earnings_history_coverage"),
    env.DB.prepare("DELETE FROM dividend_refresh_state"),
    env.DB.prepare("DELETE FROM alpha_vantage_daily_usage"),
