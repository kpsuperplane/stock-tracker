...
describe("Queue consumer", () => {
  it("acknowledges and persists a qualifying analysis", async () => {
    ...
    vi.spyOn(WorkersAiExplanationProvider.prototype, "explain").mockResolvedValue({
    ...
    const message = {
      body: { screeningId: run.screeningIds[0]! },
      ack: vi.fn(),
      retry: vi.fn(),
    } as unknown as Message<{ screeningId: string }>;
    await handleQueue(
  ...
  it("records and acknowledges a terminal provider failure", async () => {
    ...
    expect(
      await env.DB
        .prepare("SELECT status, error_code FROM screenings WHERE id = ?1")
        .bind(run.screeningIds[0])
        .first(),
