-- Importing the same CSV again is a valid user action. Keep the digest as
-- metadata, but do not use it as a uniqueness guard for import batches.
...
-- Recreate the child after the parent has its final name so SQLite preserves
-- the correct foreign-key metadata after the parent-table rebuild.
