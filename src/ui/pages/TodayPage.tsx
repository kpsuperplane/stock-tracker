...
export const todayErrorMessageKey = (
  error: unknown,
): "todayReadModelDisabled" | "todayLoadError" =>
  error instanceof ApiClientError && error.code === "read_model_disabled"
    ? "todayReadModelDisabled"
    : "todayLoadError";
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (requestId === requestIdRef.current) {
        const messageKey = todayErrorMessageKey(caught);
        setReadModelDisabled(messageKey === "todayReadModelDisabled");
    ...
    <VStack gap={3} data-testid="today-page">
      ...
      {error && (
        <Banner
          ...
          {...(readModelDisabled
            ? {
                description: t("todayReadModelDisabledDescription"),
              }
            : {})}
          endContent={
      ...
      {loading && !portfolio && (
        <Banner status="info" title={t("loadingToday")} />
      )}
